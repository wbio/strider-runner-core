...
Job.prototype = {
   ...
   */
  cmd: function (cmd, plugin, next, nextCommand) {
    ...
    if (plugin === 'node' && cmd.command === 'npm' && cmd.args[0] !== 'install' && !self.npmInstalled) {
      // We haven't run 'npm install' yet - go ahead and do that
      self.comment('Triggering \'npm install\' before running \'npm ' + cmd.args[0] + '\'');
      return self.cmd({ command: 'npm', args: ['install', '--color-always'] }, 'node', next, cmd);
    }
    ...
    this.config.spawn(cmd.command, cmd.args, options, function (err, proc) {
      ...
      proc.on('exit', function(exitCode) {
        ...
        // Set npmInstalled to true if we just ran npm install
        if (plugin === 'node' && cmd.command === 'npm' && cmd.args[0] === 'install') {
          self.npmInstalled = true;
        }

        if (nextCommand) {
          // If we set a nextCommand, call this function again with that command
          self.cmd(nextCommand, 'node', next);
        } else {
